...
extension Sequence<FCPXML.AnyRole> {
    @_disfavoredOverload
...
extension Dictionary where Value == FCPXML.AnyRole {
...
extension Sequence where Element: FCPXMLRole {
    @_disfavoredOverload
    public func contains(_ element: FCPXML.AnyRole) -> Bool {
        contains(where: { $0.asAnyRole() == element })
    }
}

extension Dictionary where Value: FCPXMLRole {
    public func contains(value element: FCPXML.AnyRole) -> Bool {
...
extension Sequence where Element: FCPXMLRole {
    public func filter(roleTypes: Set<FCPXML.RoleType>) -> [Element] {
...
extension FCPXML {
    ...
    static func _parseRawStandardRole(
        ...
        guard roleAndSubrole.count == 4, // we burn a capture group for the period (.)
            ...
            throw FCPXML.ParseError.general(
