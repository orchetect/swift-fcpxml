...
extension FCPXML.Marker {
    ...
    public init(
        name: String,
        configuration: FCPXML.Marker.Configuration,
        start: Fraction,
...
extension FCPXML.Marker: FCPXMLElementRequiredStart { }

extension FCPXML.Marker: FCPXMLElementOptionalDuration { }
...
extension XMLElement {
    ...
    public var fcpIsCompleted: Bool? {
        get {
            getBool(forAttribute: FCPXML.Marker.Attributes.completed.rawValue)
        }
        set {
            set(
                bool: newValue,
                forAttribute: FCPXML.Marker.Attributes.completed.rawValue
            )
        }
    }
    ...
    public var fcpPosterOffset: Fraction? {
        get {
            _fcpGetFraction(
                forAttribute: FCPXML.Marker.Attributes.posterOffset.rawValue,
                scaled: true
            )
        }
        set {
...
extension XMLElement {
    ...
    public var fcpMarkerConfiguration: FCPXML.Marker.Configuration? {
        get {
        ...
        set {
            ...
            if newValue.markerElementType != markerElementType {
                ...
                case .marker:
                    removeAttribute(forName: FCPXML.Marker.Attributes.posterOffset.rawValue)
                case .chapterMarker:
...
extension Sequence<FCPXML.Marker> {
    /// Sort collection by marker `start` attribute.
    public func sorted() -> [FCPXML.Marker] {
        sorted { lhs, rhs in
            lhs.start < rhs.start
        }
    }
