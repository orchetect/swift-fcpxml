...
extension XMLElement {
    ...
    func _fcpTCFormatForElementOrAncestors() -> FCPXML.TimecodeFormat? {
        let attributeName = FCPXML.TimecodeFormat.attributeName

        guard let (_, tcFormatValue) = ancestorElements(includingSelf: true)
            .first(withAttribute: attributeName),
            let tcFormat = FCPXML.TimecodeFormat(rawValue: tcFormatValue)
        else { return nil }
...
extension XMLElement {
    ...
    func _fcpCalculateAbsoluteStart(
        ancestors: (some Sequence<XMLElement>)? = nil as [XMLElement]?,
    ...
    func _fcpConformRateScalingFactor(
        ancestors: (some Sequence<XMLElement>)? = nil as [XMLElement]?,
    ...
    func _fcpFirstAncestorWithZeroLane(
        ancestors: (some Sequence<XMLElement>)? = nil as [XMLElement]?,
        includingSelf: Bool
    ...
    ///
    /// Ancestors are ordered nearest to furthest.
    func _fcpFirstContainerAncestorWithZeroLane(
        ancestors: (some Sequence<XMLElement>)? = nil as [XMLElement]?,
        includingSelf: Bool
    ) -> (element: XMLElement, remainingAncestors: AnySequence<XMLElement>)? {
        var ancestors = ancestorElements(overrideWith: ancestors, includingSelf: includingSelf)
        let types: Set<FCPXML.ElementType> = // .allTimelineCases
        [
            /* .refClip, .syncClip, .mcClip, */ .sequence,
            .spine,
            .mcAngle
        ]
