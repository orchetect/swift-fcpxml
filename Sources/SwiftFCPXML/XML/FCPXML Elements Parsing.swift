...
extension XMLElement {
    ...
    func _updateFirstChildElement(
        ofType childType: FCPXML.ElementType,
...
extension XMLElement {
    ...
    public func fcpAncestorClip<S: Sequence<XMLElement>>(
        ...
        let clipTypes = FCPXML.ElementType.allClipCases
        return ancestors
    ...
    > {
        ...
        return ancestors
            .lazy
            .compactMap { ancestor -> (type: FCPXML.ElementType, lane: Int?)? in
...
extension XMLElement {
    ...
    func _fcpFirstChildTimelineElement(
        excluding: Set<FCPXML.ElementType> = []
    ) -> XMLElement? {
        fcpTimelineElements
