...
extension XMLElement {
    ...
    func _fcpLocalRoles(
        ...
        var localRoles: [(role: FCPXML.AnyRole, isInherited: Bool)] = []

        func add(role: FCPXML.AnyRole, isInherited: Bool) {
            localRoles.append((role: role, isInherited: isInherited))
        }

        func add(roles: [FCPXML.AnyRole], isInherited: Bool) {
            for role in roles {
        ...
        case .caption:
            if let role = fcpRole(as: FCPXML.CaptionRole.self) {
        ...
        case .mcClip:
            ...
            if let videoAngle {
                let videoRoles = videoAngle.element
                    ._fcpRolesForNearestDescendant(
                        resources: resources,
                        auditions: auditions,
                        mcClipAngles: mcClipAngles,
            ...
            if let audioAngle {
                let audioRoles = audioAngle.element
                    ._fcpRolesForNearestDescendant(
                        resources: resources,
                        auditions: auditions,
                        mcClipAngles: mcClipAngles,
        ...
        case .syncClip:
            ...
            let activeAudioRoleSources = audioRoleSources.filter(\.active)
        ...
        let mappedRoles: [FCPXML.AnyInterpolatedRole] = localRoles.map {
            $0.isInherited
                ? .inherited($0.role)
                : .assigned($0.role)
        }
    ...
    func _fcpRolesForNearestDescendant(
        ...
        types: Set<FCPXML.RoleType> = .allCases,
        ignoring ignoreRoles: some Sequence<FCPXML.AnyRole>
    ) -> [FCPXML.AnyRole] {
        var collectedRoles: [FCPXML.AnyRole] = []

        func add(_ newRoles: some Sequence<FCPXML.AnyRole>) {
        ...
        func localRoles(for element: XMLElement) -> some Sequence<FCPXML.AnyRole> {
...
extension FCPXML.AudioRole {
    static let defaultAudioRole: Self = .init(rawValue: "Dialogue")!
}

extension FCPXML.VideoRole {
    static let defaultVideoRole: Self = .init(rawValue: "Video")!
    static let titlesRole: Self = .init(rawValue: "Titles")!
}
...
extension FCPXML {
    ...
    static func _defaultRoles(for elementType: ElementType) -> [AnyRole] {
        ...
        case .mcClip:
            ...
            [defaultVideoRole, defaultAudioRole]
        case .refClip:
    ...
    static func addDefaultRoles(
        for elementType: FCPXML.ElementType,
        to localRoles: [FCPXML.AnyRole]
    ) -> [FCPXML.AnyInterpolatedRole] {
        let localRoles: [FCPXML.AnyInterpolatedRole] = localRoles.map { .assigned($0) }
        return addDefaultRoles(for: elementType, to: localRoles)
    }

    static func addDefaultRoles(
        for elementType: FCPXML.ElementType,
        to localRoles: [FCPXML.AnyInterpolatedRole]
    ) -> [FCPXML.AnyInterpolatedRole] {
        var localRoles: [FCPXML.AnyInterpolatedRole] = localRoles
...
extension XMLElement {
    ...
    func _fcpDefaultRolesForNearestDescendant() -> [FCPXML.AnyRole] {
        ...
        return FCPXML._defaultRoles(for: elementType)
    }
}
