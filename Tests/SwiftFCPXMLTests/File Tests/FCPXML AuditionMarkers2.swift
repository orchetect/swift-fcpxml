...
@Suite struct FCPXML_AuditionMarkers2: TestUtils {
    // MARK: - Test Data
    
    var fileContents: Data { get throws {
        try TestResource.FCPXMLExports.auditionMarkers2.data()
    } }
    ...
    func parse() async throws {
        ...
        #expect(spine.storyElements.count == 2)
        
        let storyElements = spine.storyElements.zeroIndexed
        
        let audition = try #require(storyElements[safe: 1]?.fcpAsAudition)
        #expect(audition.offsetAsTimecode() == Self.tc("01:00:01:00", .fps29_97))
        ...
        #expect(audition1.startAsTimecode() == Self.tc("01:00:02:00", .fps29_97))
        #expect(audition1.startAsTimecode()?.frameRate == .fps29_97)
        #expect(audition1.durationAsTimecode() == Self.tc("00:00:08:00", .fps29_97))
    ...
    func extractMarkers_allAuditions() async throws {
        ...
        #expect(marker1.value(forContext: .absoluteStartAsTimecode()) == Self.tc("01:00:04:00", .fps29_97))
        #expect(marker1.value(forContext: .parentType) == .title)
        #expect(marker1.value(forContext: .parentName) == "Basic Title 1")
        #expect(marker1.value(forContext: .parentAbsoluteStartAsTimecode()) == Self.tc("01:00:01:00", .fps29_97))
        #expect(marker1.value(forContext: .parentDurationAsTimecode()) == Self.tc("00:00:08:00", .fps29_97))
        #expect(marker1.value(forContext: .ancestorEventName) == "Test Event")
        #expect(marker1.value(forContext: .ancestorProjectName) == "AuditionMarkers2")
        
        let marker2 = try #require(extractedMarkers.zeroIndexed[safe: 1])
        #expect(marker2.name == "Marker 2")
        #expect(marker2.value(forContext: .absoluteStartAsTimecode()) == Self.tc("01:00:03:00", .fps29_97))
        #expect(marker2.value(forContext: .parentType) == .title)
        #expect(marker2.value(forContext: .parentName) == "Basic Title 2")
        #expect(marker2.value(forContext: .parentAbsoluteStartAsTimecode()) == Self.tc("01:00:01:00", .fps29_97))
        #expect(marker2.value(forContext: .parentDurationAsTimecode()) == Self.tc("00:00:10:00", .fps29_97))
        #expect(marker2.value(forContext: .ancestorEventName) == "Test Event")
        #expect(marker2.value(forContext: .ancestorProjectName) == "AuditionMarkers2")
    }
}
