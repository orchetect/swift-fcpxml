...
@Suite
struct FCPXML_SyncClipRoles: TestUtils {
    // MARK: - Test Data
    ...
    func parse() async throws {
        ...
        #expect(try extractedMarker.value(forContext: .inheritedRoles) == [
            .inherited(#require(.video(raw: "VFX.VFX-Background"))), // from first video asset in sync clip
            .inherited(#require(.audio(raw: "dialogue.MixL"))), // from asset clip sync-source
            .inherited(#require(.audio(raw: "dialogue.MixR"))) // from asset clip sync-source
        ])
    }
}
